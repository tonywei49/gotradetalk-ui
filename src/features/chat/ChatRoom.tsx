    ...
    return (
        <div className={`flex w-full mb-4 ${msg.isMe ? "justify-end" : "justify-start"}`}>
                ...
                <div className="flex items-end gap-2">
                    ...
                    {msg.isMe && (
                        <div className="flex flex-col items-end justify-end text-[10px] text-gray-400 min-w-[40px] mb-1">
                            {msg.read && <span className="text-[#2F5C56] font-medium">Read</span>}
                            <span>{msg.time}</span>
                        </div>
                    )}
            ...
            `}
                    ...
                    {!msg.isMe && (
                        <span className="text-[10px] text-gray-400 self-end mb-1">{msg.time}</span>
                    )}
                </div>
            </div>
        </div>
        ...
        <div className="flex flex-col h-full w-full">
            ...
            <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0 shadow-sm z-10">
                <div className="flex flex-col">
                    <h2 className="text-lg font-bold text-slate-800">Alice Chen</h2>
                ...
                <div className="flex items-center gap-4 text-gray-500">
                    <button className="hover:text-[#2F5C56] transition-colors p-2 rounded-full hover:bg-gray-50">
            ...
            <div className="bg-white border-t border-gray-200 p-4 flex-shrink-0">
                {/* Toolbar */}
                <div className="flex gap-4 mb-2 px-1 text-gray-400">
                    <button className="hover:text-[#2F5C56]">
                ...
                <div className="flex gap-3 items-end">
                    <textarea
                        className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-slate-800 focus:outline-none focus:border-[#2F5C56] focus:ring-1 focus:ring-[#2F5C56] resize-none h-12 max-h-32 transition-all"
                        placeholder="Type a message..."
                        rows={1}
                    />
                    <button className="bg-[#2F5C56] hover:bg-[#244a45] text-white p-3 rounded-xl shadow-md transition-colors flex items-center justify-center">
